...
.DESCRIPTION
    ...
    This app's dependency set is lighter, but a vendored tree is still just
    files - what was tested is what ships - and one packaging recipe across
    the PDC-Demo apps beats a second one that can drift.

    Installs BOTH requirement sets: the web app's (requirements.txt) and the
    MCP server's (requirements-mcp.txt), so the installed app can also be
    wired into Claude Desktop without touching pip.
...
$reqFile    = Join-Path $srcRoot "requirements.txt"
$reqMcp     = Join-Path $srcRoot "requirements-mcp.txt"
...
if (-not (Test-Path -LiteralPath $reqMcp)) {
    throw "requirements-mcp.txt not found at $reqMcp"
}

# The stamp covers the runtime AND both requirement sets: a dependency added
# without a Python bump must still trigger a rebuild, or the installer silently
# ships the previous dependency set.
$reqHash = (Get-FileHash -LiteralPath $reqFile -Algorithm SHA256).Hash
$mcpHash = (Get-FileHash -LiteralPath $reqMcp -Algorithm SHA256).Hash
$stamp = "$Version+$reqHash+$mcpHash"
...
& $py -m pip install --no-warn-script-location --disable-pip-version-check -r $reqMcp
if ($LASTEXITCODE -ne 0) { throw "pip install -r requirements-mcp.txt failed" }
...
& $py -c "import uvicorn, fastapi, dotenv, mcp; print('uvicorn', uvicorn.__version__)"
if ($LASTEXITCODE -ne 0) { throw "the vendored runtime cannot import uvicorn/fastapi/dotenv/mcp" }
