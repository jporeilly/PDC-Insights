...
.DESCRIPTION
    ...
    The certificate is never in this repo. It is read from the environment:
        ...
        INSIGHTS_SIGN_THUMBPRINT  SHA-1 thumbprint of a cert in the Windows store
        PDCG_SIGN_THUMBPRINT      accepted too, so ONE variable signs every
                                  PDC-Demo suite build on this machine
        INSIGHTS_SIGN_TIMESTAMP / PDCG_SIGN_TIMESTAMP
...
$thumb = $env:INSIGHTS_SIGN_THUMBPRINT
if (-not $thumb) { $thumb = $env:PDCG_SIGN_THUMBPRINT }
if (-not $thumb) {
    Write-Host "  [--] not signing $(Split-Path -Leaf $Path) - INSIGHTS_SIGN_THUMBPRINT / PDCG_SIGN_THUMBPRINT is not set"
    exit 0
}

$timestamp = $env:INSIGHTS_SIGN_TIMESTAMP
