...
InstType "Full (app, Ollama model, environment check)"
...
Function .onInit
  ...
  ; Apply /NoOllama and /NoCheck (defined after the
...
; mode with /NoOllama and /NoCheck.
...
Section "Ollama AI runtime (local model)" SecOllama
  SectionIn 1
  ; Optional on purpose: the app also drives Anthropic and OpenAI/Azure from
  ; its Settings page, and the chat builder has a deterministic fallback - so
  ; a machine without Ollama is a configuration choice rather than a broken
  ; install.
...
!insertmacro MUI_FUNCTION_DESCRIPTION_BEGIN
  ...
  !insertmacro MUI_DESCRIPTION_TEXT ${SecOllama} "Local AI runtime. Installed only if missing, then pulls the one model sized to this machine's GPU or RAM. Skip it if you use a hosted provider - the chat builder also works with no model at all."
