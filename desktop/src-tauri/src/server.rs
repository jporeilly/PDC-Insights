...
//
//   1. A FREE PORT, chosen at launch. 5002 is the app's usual port and a
...
//   3. STATE OUTSIDE THE INSTALL. INSIGHTS_STATE_DIR is set explicitly to the
//      per-user data directory rather than left to the app's own fallback, so
//      the packaged build never depends on probing Program Files. Saved
//      dashboards, the Settings .env and the audit log all land there
//      (see app/paths.py in the repo).
...
impl Server {
    ...
    pub fn start(
        ...
        cmd.args(&args)
            .current_dir(app_dir)
            .env("INSIGHTS_STATE_DIR", state_dir)
