...
- **33 built-in dashboards** (was 18). The Quality section now carries one
  dashboard per **DQ best-practice dimension** — Completeness, Accuracy,
  Validity, Uniqueness, Consistency, Timeliness, Traceability, Clarity,
  Availability — each with the dimension's score (gauge + per-source, from a
  new `dq_<dimension>` query that serves every panel kind), the *operational
  companion panel* that moves the dimension, a worst-first fix queue, and a
  definition. Plus one operational addition per section: *DQ program*
  (Overview), *Source operations* (System), *Ownership program* (User),
  *Trust deep-dive* and *Glossary adoption* (Governance), *PII deep-dive*
  (Sensitivity). All generated by `tools/build_dashboards.py`,
  schema-validated, and mirrored as baked renders — the DQ boards' baked
  numbers are GENERATED from the same resolvers that answer /resolve
  (`tools/bake_dq_data.py` → `frontend/src/data/dqDemo.json`), so artwork and
  live overlay always agree. The `dq_dimensions` radar now carries all nine.
- **Explain results.** Every dashboard header gains an ✦ Explain button:
  `POST /api/dashboards/explain` resolves the spec under the current
  scope/demo choice and returns a markdown narrative — written by the
  configured LLM when one answers (`app/explain.py`), or a deterministic
  walk-through of the actual numbers otherwise, so it works offline. The DQ
  boards' explanations name the operational lever that moves the dimension.
- **Richer demo data.** Eight sample sources (was five; counts still sum to
  the headline totals) and an explicit `dq` block telling a deliberate story:
  Availability/Uniqueness strong, Timeliness dragged by batch sources,
  Traceability weakest — matching the Governance boards' lineage story.
- **Live mode derives instead of zeroing.** A thin or freshly-seeded PDC 11
  gave the live snapshot sources with no quality/coverage fields, and every
  quality board rendered zeros ("Mean quality 0" — the radar-with-no-data
  report). Missing per-source fields now derive stable, clearly demo-shaped
  values (the module's documented derived-tier rule); real values win the
  moment the live read supplies them. `data_sources()` maps PDC 11 root
  entities properly (connector type from `metadata.resource.type`,
  `updatedAt` as scan recency, `rootId` kept for future filtered reads —
  verified against live 11.0), and the snapshot derives per-source asset
  stand-ins so proportions render.
- **Installed-app fixes** (from the first 1.16.0 laptop install):
  - Model picker: clicking the field now always lists the installed models —
    typing filters, a preset that isn't installed no longer hides all 15
    behind "No match".
  - PDC connection: a **Verify TLS certificate** control on the Settings page
    (persists as `PDC_VERIFY_TLS`), and `test-pdc` honours the form's choice
    instead of the saved one — a self-signed lab PDC is now testable from the
    UI instead of a hand-edited .env.
  - The header version pill reads the RUNNING backend's release from
    `/health` (the bundle's package.json label had drifted to v1.12.0).
  - `model_advice`: the ≥20 GB VRAM tier recommended a 32B model while its
    own reasoning said 14B is plenty; it now recommends 14B.
  - `index.html` is served `Cache-Control: no-cache` so an upgrade can't keep
    serving the previous release's UI from the browser cache (the Glossary
    app hit exactly this, twice).
- `tools/test_app.py`: [1b] now asserts the 33-dashboard split and the nine
  dimension boards; new [3k] covers the dq_* query shapes (kpi/chart/table
  from one query), the deterministic explain path, /health's version, and
  test-pdc's verify_tls passthrough.
