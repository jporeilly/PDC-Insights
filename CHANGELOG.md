...
- **Six new built-in dashboards** — every Analytics section now ships three,
  and each new one demonstrates a spec pattern the section's first two don't:
  - *Executive scorecard* (Overview) — targets & posture on one page: bullet
    vs target + DQ radar + coverage gauge + a compact watchlist table.
  - *Scan operations* (System) — ops/SLA view: daily scan-volume line, a
    profiled-% gauge, and a full-width fix queue.
  - *Contribution pulse* (User) — GitHub-style edit-activity calendar plus an
    edits-by-action donut and an ownership gauge.
  - *Governance SLA* (Governance) — a wall of three coverage gauges
    (terms / policy / lineage) with a per-source bullet drill-down.
  - *Quality posture* (Quality) — mean-score gauge + score-band donut over a
    full-width remediation fix list.
  - *Protection controls* (Sensitivity) — twin encryption/masking gauges over
    the PII protection table.
  All 18 are generated by `tools/build_dashboards.py` (schema-validated at
  build time), listed in `app/dashboards/index.json`, and every panel binds
  to real Query Library feeds — no new queries were invented.
- React UI: the six are mirrored into `frontend/src/data/dashboards.jsx` as
  baked renders, same as the original twelve (the baked mocks carry sparkline
  /pill artwork the `/api/dashboards` index can't supply, and they keep the
  UI working fully offline; live values still overlay via
  `POST /api/dashboards/resolve`). `KPI_QUERY` gained seven exact-meaning
  labels (mean quality, term/policy coverage, lineage, owned, encrypted,
  masked) so more headline tiles show live values.
- `tools/test_app.py`: new section **[1b]** validates every generated
  `.studio.json` against the schema *and* the query catalog (18 expected,
  3 per section, index checked), and the post-run cleanup keeps the six
  new ids.
