...
the spec schema and the Analytics nav. **33 built-ins** ship, generated by
`tools/build_dashboards.py` and listed in `app/dashboards/index.json`: three
per section, one operational addition per section (two in Governance), and —
in Quality — one dashboard per **DQ best-practice dimension** (see below).
Every dashboard has an **Explain results** button that narrates the resolved
numbers (the configured LLM when one answers, a deterministic walk-through
otherwise).
...
Built-ins: *Catalog health* · *Risk hotspots* · *Executive scorecard* · *DQ program*.
...
Built-ins: *Profiling health* · *Source inventory* · *Scan operations* · *Source operations*.
...
Built-ins: *Stewardship* · *Activity & ratings* · *Contribution pulse* · *Ownership program*.
...
Built-ins: *Glossary coverage* · *Policy & lineage* · *Governance SLA* · *Trust deep-dive* · *Glossary adoption*.
...
Built-ins: *Quality scores* · *DQ dimensions* · *Quality posture*, plus one
...
Availability** (12 in this section).

Each dimension dashboard follows the same teaching shape — the dimension's
score (gauge + per-source bars, from its own `dq_<dimension>` query), the
**operational companion panel** that actually moves the dimension (profiling
for Completeness, lineage for Traceability, scan activity for Timeliness, …),
the worst-first fix queue, and a definition of what the dimension measures.
The `dq` block in the sample snapshot tells a deliberate story: Availability
and Uniqueness strong, Timeliness dragged by the batch sources, Traceability
weakest — matching what the Governance boards say about lineage.
...
Built-ins: *Exposure overview* · *PII discoveries* · *Protection controls* · *PII deep-dive*.
...
Live mode also has a **per-view demo override**: the source-scope dropdown's
"── Demo data (sample)" entry renders just that view (and its drill-through)
from the bundled sample via an optional top-level `"demo": true` on
`POST /api/dashboards/resolve` and `/drill` — per-request, read-only, and the
app-wide `INSIGHTS_DEMO` setting is never touched.
...
Besides the 18 built-ins, dashboards can be built conversationally in the app at
